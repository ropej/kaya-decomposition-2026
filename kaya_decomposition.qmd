...
plot_kaya <- function(data, nazev_statu, interaktivni = FALSE) {
  ...
  if (interaktivni) {
    p_ly <- ggplotly(p, tooltip = "text") |> config(displayModeBar = FALSE)
    seen <- character()
...
```{r tbl-income-classification}
#| echo: false
#| tbl-cap: "Rozdělení zemí podle příjmové skupiny Světové banky."
datasummary_skim(
  gdp_pc_ref |> 
    select(income_group) |>
    mutate(across(c(income_group), ~ remove_accents(.x))),
  escape = FALSE
  )
```
...
```{r tbl-fossil-classification}
#| echo: false
#| tbl-cap: "Zastoupení zemí podle role v mezinárodním obchodu s fosilními palivy."
datasummary_skim(
  exporteri_owid |> 
    select(vyvoz_ropa, vyvoz_plyn, vyvoz_uhli, fossil_role) |>
    mutate(across(c(fossil_role), ~ remove_accents(.x))),
  escape = FALSE
  )
```
...
kaya |>
  ...
  pivot_longer(c(d_pop, d_gdp_pc, d_ei, d_ci),
               names_to = "faktor", values_to = "hodnota") |>
  mutate(faktor = preloz_faktory(faktor),
         faktor = factor(faktor, levels = names(barvy_kaya)[1:4])) |>
  ...
  scale_fill_manual(values = barvy_kaya[1:4], name = NULL) +
  scale_color_manual(values = c("Fosilní CO₂" = "#222222"), name = NULL) +
...
ggplotly(p_scatter, tooltip = "text") |>
  ...
  config(displayModeBar = TRUE, displaylogo = FALSE,
         modeBarButtonsToRemove = c("toImage", "pan2d", "select2d", "lasso2d",
...
p_vzd <- ggplot(vzd_data,
  ...
  labs(x = "Hrubá míra terciárního vzdělání, průměr 2010–2019 (%)",
       y = "Průměrná roční změna CO₂ (% /rok, 1990–2024)",
       color = NULL) +
  theme(legend.position = "bottom")
...
cluster_data |>
  ...
  scale_fill_manual(values = barvy_kaya[1:4]) +

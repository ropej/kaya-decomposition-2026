...
edu_wdi <- if (requireNamespace("WDI", quietly = TRUE)) {
  tryCatch(
    WDI::WDI(indicator = c(tertiarni = "SE.TER.ENRR"),
      ...
      summarise(terciarni_vzdelanost = mean(tertiarni, na.rm = TRUE),
                       .groups = "drop"),
    error = function(e) tibble::tibble(iso_code = character(),
                                       terciarni_vzdelanost = numeric())
  )
} else {
  message("WDI není nainstalovaný – data vzdělání chybí. Spusť: install.packages('WDI')")
  tibble::tibble(iso_code = character(), terciarni_vzdelanost = numeric())
}
...
Součet čtyř faktorů se nemusí přesně rovnat skutečné změně emisí CO₂. Rozdíl tvoří **interakční člen**, který zachycuje nelineární souběh změn jednotlivých faktorů.

$$\text{Interakce} = \Delta \ln \text{CO}_2 - (\Delta \ln P + \Delta \ln \frac{\text{HDP}}{P} + \Delta \ln \frac{E}{\text{HDP}} + \Delta \ln \frac{\text{CO}_2}{E})$$

Ve většině let je tento rozdíl blízký nule, protože při malých meziročních změnách je logaritmická aproximace velmi přesná. Výraznější odchylky se objevují hlavně v letech s náhlými šoky, například během pandemie COVID-19 v roce 2020.
...
income_data <- gdp_pc_ref |>
  ...
  mutate(
    ...
    text = paste0("Příjmová skupina: ", income_group,
                  "<br>Počet zemí: ", n,
                  "<br>Podíl: ", format(round(pct, 2), nsmall = 2), " %")
  )
...
vzd_data <- cluster_data |>
  filter(!is.na(terciarni_vzdelanost), !is.na(co2_mean), !is.na(income_group)) |>
  mutate(
    popisek = if_else(country %in% c(vybrane, "Czechia"), country, NA_character_),
    text    = country
  )
...
lm_vzd  <- lm(co2_mean ~ terciarni_vzdelanost, data = vzd_data)
lm_seq  <- seq(min(vzd_data$terciarni_vzdelanost, na.rm = TRUE),
               max(vzd_data$terciarni_vzdelanost, na.rm = TRUE),
               length.out = 200)
lm_pred <- predict(lm_vzd,
                   newdata = data.frame(terciarni_vzdelanost = lm_seq),
                   interval = "confidence") |>
  as.data.frame() |>
  mutate(terciarni_vzdelanost = lm_seq)

p_vzd <- ggplot(vzd_data,
                aes(x = terciarni_vzdelanost, y = co2_mean, color = income_group)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray60") +
  geom_ribbon(data = lm_pred,
              aes(x = terciarni_vzdelanost, ymin = lwr, ymax = upr),
              inherit.aes = FALSE, fill = "gray30") +
  geom_line(data = lm_pred,
            aes(x = terciarni_vzdelanost, y = fit),
...
cluster_data <- kaya |>
  ...
  left_join(klasifikace_kultura |> 
              select(iso_code, jazyk_skupina, nabozenstvi, terciarni_vzdelanost),
            by = "iso_code") |>
...
cluster_data |>
  ...
  summarise(
    ...
    `Terciární vzdělání (% med.)`   = round(median(terciarni_vzdelanost, na.rm = TRUE), 0),
    `Kontinent (%)`                  = {
